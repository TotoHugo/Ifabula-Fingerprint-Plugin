...
public class FingerPrintPlugin2 extends CordovaPlugin {
	...
	private int mCmdSize = 0;
	 ...
	 public String mTempData = "";  // match FP template data
	 public byte mTempDataByte[] = new byte[512]; 
	...
	public boolean execute(String action, CordovaArgs args, final CallbackContext callbackContext) throws JSONException {
		...
		if (action.equals("registerAdapterStateChanged")) {
			...
			int socketId = args.getInt(0);
			// mTempData = args.getString(1);
			mTempDataByte = args.getArrayBuffer(1);
				...
				if (bytesRead < 0) {
					...
					if (mDeviceCmd == CMD_GETIMAGE) { 
						...
						if (mUpImageSize >= 36864) {
							...
							data = Arrays.copyOf(mUpImage, mUpImageSize);
					...
					}else if(mDeviceCmd == CMD_ENROLHOST || mDeviceCmd == CMD_CAPTUREHOST){
						memcpy(mCmdData, mCmdSize, readBuffer, 0, bytesRead);
						mCmdSize = mCmdSize + bytesRead;
								...
								switch (mCmdData[4]) {
									case CMD_ENROLHOST: {
										...
										if (mCmdData[7] == 1) {
											...
											// String encoded = Base64.encodeToString(mRefData, Base64.DEFAULT);
									...
									case CMD_CAPTUREHOST: {
										...
										if (mCmdData[7] == 1){
											...
											// byte[] decoded = mTempData.getBytes();
											// byte [] decoded = Base64.decode(mTempData, Base64.DEFAULT);
											int ret = FPMatch.getInstance().MatchFingerData(mTempDataByte,
													mMatData);
     ...
     */
    private void SendCommand(CallbackContext callbackContext, BluetoothSocket socket,byte cmdid, byte[] data, int size) {
